...
const ExoplanetVisualization = ({ features }) => {
  ...
  useEffect(() => {
    ...
    // Añadir etiquetas
    scene.add(createTextSprite('Estrella Anfitriona', -15, starRadius + 3, 0, 15));
    scene.add(createTextSprite(`${sradSolar.toFixed(2)} x Sol`, -15, -starRadius - 2.5, 0, 12));
    
    scene.add(createTextSprite('Exoplaneta', 0, planetRadius + 3, 0, 15));
    scene.add(createTextSprite(`${pradEarth.toFixed(2)} x Tierra`, 0, -planetRadius - 2.5, 0, 12));
    scene.add(createTextSprite(planetDesc, 0, -planetRadius - 4.5, 0, 10));
    
    scene.add(createTextSprite('Tierra', 15, earthRadius + 3, 0, 15));
    scene.add(createTextSprite('(Referencia)', 15, -earthRadius - 2.5, 0, 12));
  ...
  return (
    <div className="relative">
      ...
      {/* Overlay con información del planeta */}
      {features && features.koi_period && (
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-70 text-white px-8 py-4 rounded-lg text-center">
          <div className="text-lg font-semibold">
            Un año en este planeta dura {parseFloat(features.koi_period).toFixed(1)} días terrestres
          </div>
        </div>
      )}
    </div>
